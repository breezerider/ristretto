...
- **`/ristretto:brew`** — brew the whole pot: autonomously pulls every eligible planned feature in sequence — same gates, evidence, and close as `pull`, one commit per feature on a single `feature/brew-<date>` session branch. **Each feature runs in a fresh subagent**: the main conversation stays a small orchestrator no matter how big the batch, and every implementation sees exactly one spec instead of the drift of the whole run. Anything needing a decision gets status `blocked` with a one-line reason instead of a guess; you walk through `status blocked` afterward, refine, and re-brew. For when you've prepped a batch and don't want to babysit the roadmap.
