...
- **`/ristretto:migrate`** — brings `docs/ristretto/` up to the current format. Every command checks the roadmap's format stamp before doing anything and runs this first if the project is behind, telling you before it touches a file. Shape only: no criterion, decision, or status *meaning* is altered, and no checkbox is ticked or un-ticked.
...
So `roadmap.md` carries `<!-- ristretto-format: 0.14 -->`, and every command begins by comparing it against the plugin — `node scripts/version.js check`, a fact established in code rather than eyeballed. Behind means `/ristretto:migrate` runs first, announcing itself before it touches anything. Ahead means your *install* is stale, and it refuses to migrate rather than rewrite your files into an older shape. The format version is the plugin's MAJOR.MINOR, so a patch release never triggers a migration. Check it with `node scripts/version.js check` and `node scripts/version.test.js`.
