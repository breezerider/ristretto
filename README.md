...
- **A feature carries a tier**: `normal` (the default — plan, build, review, close) or `easy`, which **skips the planner and nothing else**. `easy` is a claim about the *contract*, not the ticket: it means the contract is already concrete enough that a planner would add nothing to it, so the label is checkable by reading the contract rather than guessing at the work. `prep` sets it and must give a one-clause reason for `easy`; `normal` needs none. **The ratchet is one-way** — any agent that reads the code may escalate `easy` → `normal` (the contract doesn't survive contact with the code, it needs a migration, it spans more than three files, a criterion is `[human]`), and nothing may ever lower a tier. A wrong `easy` costs one implementer stopping before it writes code; a right one saves the planner. `/ristretto:brew easy` forces every feature through the easy path and records what *would* have escalated without acting on it — an experiment lane, labelled `tier: easy (forced)` on the record forever. Gates and review run identically at every tier: tiering changes ceremony, never proof.
