...
- **An independent review gates every commit**: after the gates are green, a fresh subagent that never saw the implementation reasoning judges the diff — `bug` findings (a criterion not actually met, edge cases, dishonest tests) must be fixed, `lean` findings (tamp's facets) unless riskier than the win. Two rounds max, then it surfaces instead of looping — in `brew`, an unresolved defect means `blocked`, not "done with caveats". Trivial diffs (a few lines, no new logic) skip the review.
