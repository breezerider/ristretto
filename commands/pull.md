...
>
> Report **every `bug` you can point at — that list is never truncated** — then at most **5 `lean`** findings, highest-value first; if more lean exist, add `+N minor lean nits omitted` and stop. Bugs first, each one line: `bug|lean · file:line · what's wrong · the fix`. Flag only what you can point at — no hypotheticals, no style nits. **House-rule staleness is not a finding**: if `CLAUDE.md` / `AGENTS.md` has drifted out of date with the code, put that in one trailing line *after* the findings — nothing in this loop may edit those files, so a finding nobody is allowed to act on costs a fixer round and buys nothing. (A diff that *violates* a house rule is still a `bug`.) If nothing material, your final message is exactly `review: clean`. Otherwise: `review: N findings` followed by the findings. Nothing else.
...
2. **Correct `Provides:` to whatever was actually built**, then append an `## Evidence` section to the plan (the proof from step 9, a one-line gate summary like `gates: lint ✓ typecheck ✓ test ✓`, and the review verdict **with the rounds it took** — `review: clean (1 round)`, `review: 4 findings resolved in 2 rounds`, `review: skipped (trivial diff)`, or `review: skipped (raw)`), then move `docs/ristretto/plans/<FEATURE-ID>.md` → `docs/ristretto/plans/archived/<FEATURE-ID>.md`. A `Provides:` that drifted during implementation and was never corrected poisons every dependent feature — the archived plan is what the next feature's planner reads as fact.
