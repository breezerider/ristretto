...
9. **Review gate — before any commit.** Skip only if the diff is trivial (roughly < 15 changed lines and no new functions/branches/loops; when in doubt, review — many shots will qualify for the skip, that's fine). Otherwise dispatch one fresh subagent with `pull`'s review brief verbatim: read the plan, the repo's `CLAUDE.md` / `AGENTS.md` if present, and the diff, change no files and run no gates; buckets in priority order `block` (the shipped product misbehaves — a criterion genuinely unsatisfied, data loss, a security hole, a house rule the diff violates, a reachable unhandled edge case), `note` (the product is right but the proof is weaker than it claims — a vacuously passing test, a docblock overstating what is proven), then `lean` (runtime waste, duplication vs existing utilities, dead/over-built, readability); every `block` (never truncated) then at most 5 `note` and 5 `lean`, one line each (`block|note|lean · file:line · what · fix`), each note carrying one clause on why a user cannot be harmed, with house-rule staleness as a trailing note rather than a finding, and a final line of exactly `review: clean`, `review: notes-only (n note, m lean)`, or `review: blocking (n)`. On `notes-only`, close — record the notes in the plan, no fixer. On `blocking`, fix every `block` (mandatory) plus the notes and quick leans, re-run gates; one confirming round max (2 total). Blocks still open after round 2 → do **not** commit; surface the findings, disarm the gates, stop.
10. **Close (mandatory):**
