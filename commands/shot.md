...
2. **Arm the gates.** As in `pull` step 3: if `.ristretto.json` is missing at the repo root, create it (detect the stack, adopt the repo's existing format/lint/typecheck/test tooling — never impose new tools; set `testChanged` if the suite is slow; add `.ristretto/` to `.gitignore`). Then create the marker file `.ristretto/pulling` — while it exists, the plugin's Stop hook runs lint + typecheck + the scoped test gate and blocks until green. Never weaken, skip, or delete gates or tests to get green.
