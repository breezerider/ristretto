...
2. **Read the feature's `Tier` from its roadmap row first.**
   ...
   **Check the argument before the row — it overrides every branch below.** If `easy` was passed, every feature takes the `easy` branch whatever its `Tier` cell says, and the `Tier` cell is not consulted at all. See **`easy` — the forced-easy experiment lane** above. Reading the row first is how a forced run quietly becomes an ordinary one: on a roadmap where every row is `normal` — which is every migrated project — the `normal` branch matches all of them and the lane never fires.

   - `easy`, or the `easy` argument was passed → **skip the planner entirely.** The contract is the build plan. Dispatch the implementer directly and tell it to expand the contract inline before writing anything: read the current code in the touchpoint areas, find the utilities, patterns and test conventions this repo already uses, and settle the exact file paths, the real names and signatures, and the test cases that prove each criterion. Nothing is written to `.ristretto/build/`. **Everything else in this loop is unchanged**: the gates stay armed, the tests still go red first, the review still runs, the closer still closes.
   - `normal`, or no `Tier` cell at all → dispatch the planner exactly as in the next step.

   **The ratchet — escalate, never lower.** While expanding an `easy` contract inline, the implementer stops **before writing any code** and returns `escalate: <trigger>` if any of these is true (**under forced-easy it records `would-escalate:` and builds anyway instead** — the brief below carries both arms, so hand it over verbatim and say which run this is):
   ...
   On `escalate:`, **you** dispatch the planner as in the next step, then a fresh implementer against the build plan it wrote, and you tell the closer to flip the row's `Tier` to `normal` and record `escalated from easy: <trigger>` in the plan's `## Evidence`. An escalation costs one implementer's startup, not a round: nothing was written, so there is nothing to undo. **Under forced-easy no `escalate:` ever arrives** — that is the lane's whole design, and a `would-escalate:` line in a result is data for the report, never a signal to dispatch a planner. **Nothing may ever lower a tier** — not this command, not a subagent, not a later run. An upfront label is an estimate made before anyone read the code, and this repo has receipts on those being optimistic; the ratchet is what makes being wrong cost minutes instead of a broken feature.
...
4. **Dispatch the implementer** (fresh context, general-purpose) — **one feature at a time, never in parallel**: subagents share one branch and one repo-wide gate. Give it this brief verbatim, filling in the ID and saying whether this is a forced-easy run. The brief carries both the planned and the easy path and decides between them by looking, so it is correct whichever branch step 2 took — **never edit it down to the path you think applies**:

   > You are implementing exactly one ristretto feature: **<FEATURE-ID>**. Work autonomously; you cannot ask the user anything.
   >
   > 1. **Find your build plan — one of two things is true, and looking tells you which.** If `.ristretto/build/<FEATURE-ID>.md` exists, that is your plan, written against the current code minutes ago: implement it, do not re-plan. If it does **not** exist, this is an `easy` feature and the contract is the plan — before writing any code, expand it yourself: read the current code in the touchpoint areas, find the utilities, patterns and test conventions this repo already uses, and settle the exact file paths, the real names and signatures, and the test cases that prove each criterion. Either way, `docs/ristretto/plans/<FEATURE-ID>.md`'s `## Contract` is the acceptance contract behind it, and below, "the build plan" means whichever of the two you have. A missing file is never an error to report — it is the easy path, and it is expected.
   ...
   >
   > **`escalate:` is available only on an `easy` feature** — one where you found no build plan in step 1 — **and it is not a failure.** It means the contract you were handed is not concrete enough to build from without a planner. Return it *before writing any code*, on any of these five triggers:
   >
   > 1. the contract cannot be satisfied as written against the current code;
   > 2. it needs a new dependency, a migration, a schema change, or a manual check the contract does not already name;
   > 3. it must create public surface not named in `Provides:`;
   ...
   > 5. any acceptance criterion is `[human]`.
   >
   > The orchestrator will have a planner write a real build plan and hand it to a fresh implementer. **Never reach for `blocked:` instead**: `blocked` means a spec gap no one can build from and it holds back every feature that depends on this one, whereas an escalation is just a tier label that was optimistic. Confusing the two costs the whole flight behind you.
   >
   > **If you were told this is a forced-easy run (`brew easy`), there is no `escalate:`.** You return `ready:` or `needs-human:` exactly as normal, and put `would-escalate: <trigger>` in your trailing lines instead — naming which of the five above fired — then build it anyway, without a plan. That line is the measurement the run exists to produce, so never omit it and never soften it. If you were told nothing about forced-easy, this is not one: return `escalate:` as above.
...
8. **Record the result — and print nothing else.** Each subagent's one-liner is all you keep. Print `☕ <FEATURE-ID> brewed (n/m)`, `🔧 <FEATURE-ID> brewed, needs a human check — <check> (n/m)`, `👀 <FEATURE-ID> brewed, needs review — <count> finding(s) open (n/m)`, or `⛔ <FEATURE-ID> blocked — <reason> (n/m)` between features so the trail is readable.

   **Every result line ends with the counter, and it means position in the run.** `n` is how many features this run has finished in *any* terminal state — brewed, needs-human, needs-review and blocked alike — so it advances by exactly one on every line and can never stall; `m` is **every `planned` row on the roadmap when the loop starts**, fixed there and never recomputed. It is deliberately not the *eligible* count: eligibility grows as prerequisites close, so a flight of three chained features starts with one eligible and would print `(2/1)`, `(3/1)` — a denominator you can overrun is worse than no denominator. Counting all `planned` rows can instead end a run at `9/12`, which is the honest reading: three never became eligible behind something that blocked, and they are named in the report. The counter says *how far through the pot is*; the emoji already says *how well it is going*. A counter that appeared only on the features that went well is how someone who stepped away ends up opening the roadmap to find out where the run is — which is the one thing this trail exists to prevent, and it is what happened before 0.16 on a run where several features closed `needs-review`.

   A feature where round 3 settled an open question adds `· decision taken: <the ruling>` to its line — that is the one thing in this loop a person might want to overrule, and it must not wait for the final report to be seen.
...
- **A subagent that dies is not the user changing their mind.** The single most convincing way this loop breaks is a dispatch that comes back empty and *reads* as an interruption — so you conclude the user is steering, and stopping for a user who is steering is correct behaviour. It is the right rule fired on a false premise, and it ends the run just as dead as breaking a rule would. You cannot distinguish the two, so never infer intent from a dead subagent, and never build a theory out of two of them. Loop step 7 says what to do instead; it is not a judgement call.
