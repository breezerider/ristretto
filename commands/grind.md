---
description: Honest refinement review of a ticket — plain-language summary, story-point estimate, the problems it actually has, and a Ready / Not-Ready verdict. Read-only.
argument-hint: <ticket: paste text/ACs, or an ID + description>
---

You are running **GRIND** — an honest refinement review of one ticket, for use in a refinement session. Read-only: assess and print, change no files.

Ticket: $ARGUMENTS

Match the ticket's language (German tickets → German output).

## Don't guess at what you can't see

If the ticket references links, internal pages (Jira, NGS knowledge base), screenshots, or attachments you can't access, **stop and ask the user to paste the content**, naming exactly what you need. Don't invent acceptance criteria or scope from an inaccessible link — a review built on guesses is worse than no review.
...
**Summary** — 2–3 sentences, plain language: what the ticket delivers and why. Written so it can be read aloud in refinement.
...
Close with a single coffee-themed line that mirrors the verdict — make it fit the specific ticket where you can:
